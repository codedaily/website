...
이 문서는 Kubernetes의 현재 `PersistentVolumes`의 상태를 설명한다. [볼륨](/docs/concepts/storage/volumes/) 참고.
...
스토리지 관리는 컴퓨팅 관리와는 별개이다. `PersistentVolume` 서브시스템은 사용자와 관리자에게
이를 위해 두 가지 새로운 API 리소스를 소개한다: `PersistentVolume` 와 `PersistentVolumeClaim`.
...
`PersistentVolumeClaim` (PVC)는 사용자의 저장 요청이다. 파드와 비슷하다. 파드는 노드 리소스를 사용하고 PVC는 PV 리소스를 사용한다. 파드는 특정 수준의 리소스(CPU와 메모리)를 요청할 수 있다. 클레임은 특정 크기와 액세스 모드를 요청할 수 있다.(예를 들어, 한번 읽기/쓰기 또는 여러 번 읽기 전용으로 마운트 가능).
...
PV는 클러스터의 리소스이다. PVC는 해당 리소스에 대한 요청이고 리소스에 대한 클레임 검사 역할을 한다. The interaction between PVs and PVCs follows this lifecycle:
